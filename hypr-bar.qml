...
ShellRoot {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ─── WiFi State (nmcli — NetworkManager + iwd) ────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ── Live SSID + signal every 5s ───────────────────────────────────────
    // nmcli -t gives us terse colon-separated output; grep the active AP.
    // One process, two lines out per tick: SSID then signal (0–100).
    Process {
        id: wifiStatusProc
        command: [
            "bash", "-c",
    ...
    // ── Scan with nmcli ───────────────────────────────────────────────────
    // rescan is async so we wait 2s, then read the network list.
    // Output: SSID:SECURITY:SIGNAL:ACTIVE — SSID may contain colons,
    // so we split from the right (last 3 fields are always well-defined).
    Process {
        id: wifiScanProc
        running: false
        stdout: SplitParser {
            ...
            onRead: (line) => {
                ...
                var active   = parts[parts.length - 1]
