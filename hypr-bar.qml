    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (line) => {
                ...
                list.push({ ssid: ssid, signal: sig, security: security, connected: inUse, icon: "\uf1eb" })
        ...
        PanelWindow {
            ...
            PanelWindow {
                ...
                Column {
                    ...
                    Item {
                        width: parent.width; height: 160; visible: root.chartData.length === 0
                    ...
                    Item {
                        ...
                        Canvas {
                            ...
                            onPaint: {
                                ...
                                for (var m = 0; m < pts.length; m++) { var isNow = data[m].label === "Now"; ctx.beginPath(); ctx.arc(pts[m].x, pts[m].y, isNow ? 4 : 2.5, 0, Math.PI*2); ctx.fillStyle = isNow ? "rgba(255,255,255,0.5)" : "rgba(160,220,255,0.5)"; ctx.fill(); if (isNow) { ctx.beginPath(); ctx.arc(pts[m].x, pts[m].y, 7, 0, Math.PI*2); ctx.strokeStyle = "rgba(255,255,255,0.22)"; ctx.lineWidth = 1.5; ctx.stroke() } }
                            }
                        }
                        Row {
                            anchors.fill: parent
            ...
            Column {
                ...
                Item { width: parent.width; height: 3
                    ...
                    Rectangle { anchors { left: parent.left; top: parent.top; bottom: parent.bottom }
                        width: parent.width * root.musicProgress; radius: 1.5; color: Qt.rgba(0.72,0.88,1.0,0.85); Behavior on width { NumberAnimation { duration: 950; easing.type: Easing.Linear } } }
                }
                Item { width: 1; height: 4 }
                ...
                Item { width: parent.width; height: 64
                    Canvas {
                        ...
                        Connections { target: root; function onCavaHeightsChanged() { cavaCanvas.requestPaint() }
                ...
                Column {
                    ...
                    RowLayout { width: parent.width; height: 30
                        ...
                        Item {
                            ...
                            Text { anchors.centerIn: parent; text: "\uf021"; font.family: "Symbols Nerd Font"; font.pixelSize: 11; color: root.wifiScanning ? Qt.rgba(0.72,0.88,1.0,0.80) : Qt.rgba(1,1,1,0.40); RotationAnimation on rotation { id: wifiSpinAnim; running: root.wifiScanning; from: 0; to: 360; duration: 900; loops: Animation.Infinite } }
                    ...
                    Item { width: parent.width; height: root.wifiConnectMsg.length > 0 ? 28 : 0; visible: root.wifiConnectMsg.length > 0; Behavior on height { NumberAnimation { duration: 150 } }
                        Text { anchors { left: parent.left; right: parent.right; verticalCenter: parent.verticalCenter }
                            text: root.wifiConnectMsg; font.family: "SF Pro Display"; font.pixelSize: 10; color: Qt.rgba(0.72,0.88,1.0,0.70); elide: Text.ElideRight }
                    }
                    ...
                    Item { width: parent.width; height: 48; visible: root.wifiScanning || root.wifiNetworks.length === 0
                        Column { anchors.centerIn: parent; spacing: 6
                            Text { anchors.horizontalCenter: parent.horizontalCenter; text: root.wifiScanning ? "\uf021" : "\uf204"; font.family: "Symbols Nerd Font"; font.pixelSize: 20; color: Qt.rgba(1,1,1,0.25); RotationAnimation on rotation { running: root.wifiScanning; from: 0; to: 360; duration: 900; loops: Animation.Infinite } }
                                    ...
                                    Column { anchors { fill: parent; margins: 10 }
                                        ...
                                        RowLayout { width: parent.width; height: 28; spacing: 8
                                            ...
                                            Rectangle { Layout.fillWidth: true; height: 28; radius: root.pillRadius; color: Qt.rgba(1,1,1,0.07); border.color: pwInput.activeFocus ? Qt.rgba(0.42,0.68,1.0,0.55) : Qt.rgba(1,1,1,0.10); border.width: 1; Behavior on border.color { ColorAnimation { duration: 130 } }
                                                TextInput { id: pwInput; anchors { fill: parent; leftMargin: 10; rightMargin: 10 }
                                                    verticalAlignment: TextInput.AlignVCenter; echoMode: TextInput.Password; font.family: "SF Pro Display"; font.pixelSize: 12; color: "white"; selectionColor: Qt.rgba(0.42,0.68,1.0,0.35); onTextChanged: root.wifiPasswordInput = text; onAccepted: { if (text.length > 0) { root.wifiConnect(modelData.ssid); root.wifiExpandedSsid = "" } } }
                                                Text { anchors { fill: parent; leftMargin: 10 }
                                                    verticalAlignment: Text.AlignVCenter; visible: pwInput.text.length === 0 && !pwInput.activeFocus; text: "Password…"; font.family: "SF Pro Display"; font.pixelSize: 12; color: Qt.rgba(1,1,1,0.25) }
                                            }
                                            Item {
                                                ...
                                                MouseArea { anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onEntered: pwConnBg.color = Qt.rgba(0.42,0.68,1.0,0.38); onExited: pwConnBg.color = Qt.rgba(0.42,0.68,1.0,0.22); onClicked: { if (root.wifiPasswordInput.length > 0) { root.wifiConnect(modelData.ssid); root.wifiExpandedSsid = "" } } }
                                            }
                                            Item {
                                                implicitWidth: 28; implicitHeight: 28
                    ...
                    Rectangle { width: parent.width; height: 1; color: Qt.rgba(1,1,1,0.06) }
                    Item { width: 1; height: 8 }
                    Text { text: "nmcli · click to connect"; font.family: "SF Pro Display"; font.pixelSize: 9; color: Qt.rgba(1,1,1,0.16) }
                    Item { width: 1; height: 4 }
                }
            }
        }
